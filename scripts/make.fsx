...
let CONSOLE_FRONTEND = sprintf "%s.Frontend.Console" SOLUTIONS_PREFIX
let GTK_FRONTEND = sprintf "%s.Frontend.XF.Gtk" SOLUTIONS_PREFIX
...
let GetProject (projFile: ProjectFile) =
    let projFileName =
        match projFile with
        | ConsoleFrontend -> sprintf "%s.Frontend.Console" SOLUTIONS_PREFIX
        | GtkFrontend -> sprintf "%s.Frontend.XF.Gtk" SOLUTIONS_PREFIX
        | XFFrontend -> sprintf "%s.Frontend.XF" SOLUTIONS_PREFIX
...
let BACKEND = sprintf "%s.Backend" SOLUTIONS_PREFIX
...
| Some("check") ->
    ...
    let testProjectName = sprintf "%s.Backend.Tests" SOLUTIONS_PREFIX
#if !LEGACY_FRAMEWORK
    let testTarget =
        Path.Combine (
            FsxHelper.RootDir.FullName,
