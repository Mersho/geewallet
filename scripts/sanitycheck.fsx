...
let projDir =
    if args.IsEmpty then
        ...
        failwithf "Directory does not exist."
...
let NugetSolutionPackagesDir =
    ...
    if File.Exists nugetPackageConfigDir then
        ...
        failwithf "NuGet.config not found in '%s'" nugetPackageConfigDir
...
module MapHelper =
    ...
    let MergeIntoMap<'K, 'V when 'K: comparison>
        (from: seq<'K * 'V>)
        : Map<'K, seq<'V>> =
